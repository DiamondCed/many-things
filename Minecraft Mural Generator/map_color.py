...
def set_color_file(filename: str):
    global color_data, color_filename, texture_filepaths
    color_data = np.load(filename.strip("\""), allow_pickle=True) # let the user deal with this error themselves
    color_filename = filename
    try:
        presets, antipresets = name_to_presets("".join(filename.split("\\")[-1].split(".")[:-1]))
...
def map_color(color: tuple, radius=256) -> list[int]:
    ...
    offset = 0
    while offset<radius: # 256 being the known size of the array in each of the 3 dimensions
        subset = color_data[
