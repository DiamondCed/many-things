...
execute as @e[tag=matter_netherite] at @s if block ~ ~-2 ~ #replicator:tier_2 run loot insert ~ ~-3 ~ mine ~ ~-2 ~ diamond_pickaxe{Enchantments:[{id:"silk_touch",lvl:1s}]}
execute as @e[tag=matter_diamond] at @s if block ~ ~-2 ~ #replicator:tier_1 run loot insert ~ ~-3 ~ mine ~ ~-2 ~ diamond_pickaxe{Enchantments:[{id:"silk_touch",lvl:1s}]}
...
schedule function replicator:replicate/30s 30s