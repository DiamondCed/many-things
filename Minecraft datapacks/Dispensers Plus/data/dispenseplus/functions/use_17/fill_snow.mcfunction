...
execute if entity @s[tag=found] run playsound item.bucket.empty_powder_snow block @a ~ ~ ~
execute if entity @s[tag=found] run stopsound @a block block.dispenser.dispense

execute if entity @s[tag=found] run setblock ~ ~ ~ powder_snow_cauldron[level=3]
